...
' ============================================================================
' GenerateWeeklyReport (v2 - matches reference formatting, real names,
...
'
' Real author names are used throughout (no anonymization). A hidden sheet
' called AuthorRoster in this workbook remembers every name ever seen, so
' the team roster (and Team Size) stays consistent week to week even if
' someone logs zero hours in a given week.
' ============================================================================
...
Sub GenerateWeeklyReport()
    ...
    ' ---- Determine the true last row of data --------------------------------
    ' FIX: previously this was computed as Cells(Rows.Count, 1).End(xlUp).Row
    ' i.e. it only looked at column A, BEFORE the real data columns were even
    ' located above. In raw exports where column A isn't one of the columns
    ' filled all the way to the bottom (very common with Jira/Tempo exports),
    ' that made lastRow too small and silently dropped every row past it -
    ' with no error, so it looked like entries just went missing.
    ' Now it takes the max across all the columns we actually read from.
    lastRow = csvWs.Cells(csvWs.Rows.Count, colDate).End(xlUp).Row
    lastRow = Application.WorksheetFunction.Max(lastRow, _
        csvWs.Cells(csvWs.Rows.Count, colTicket).End(xlUp).Row, _
        csvWs.Cells(csvWs.Rows.Count, colHours).End(xlUp).Row, _
        csvWs.Cells(csvWs.Rows.Count, colAuthor).End(xlUp).Row, _
        csvWs.Cells(csvWs.Rows.Count, colSummary).End(xlUp).Row, _
        csvWs.Cells(csvWs.Rows.Count, colStatus).End(xlUp).Row, _
        csvWs.Cells(csvWs.Rows.Count, colPriority).End(xlUp).Row)

    ' ---- Look for an issue-level sheet with real per-ticket "Created" -----
    Dim hasIssueSheet As Boolean
    Dim issueCreated As Object
    ...
    Dim ticketWorkFirstSeen As Object
    ...
    For i = 1 To n
        ...
        ElseIf dArr(i) < ticketWorkFirstSeen(tArr(i)) Then
            ticketWorkFirstSeen(tArr(i)) = dArr(i)
        End If
    Next i

    ' ticketCreated: real creation date per ticket, ONLY for tickets we have
    ' a confident date for. If hasIssueSheet, a ticket missing from it is
    ' treated as pre-existing (no entry made = "not new").
    Dim ticketCreated As Object
    Set ticketCreated = CreateObject("Scripting.Dictionary")
    For Each k In ticketSummary.Keys
        Dim tk As String
        tk = CStr(k)
        If hasIssueSheet Then
            If issueCreated.Exists(tk) Then ticketCreated.Add tk, issueCreated(tk)
            ' else: leave unset -> not treated as "new"
        Else
            ticketCreated.Add tk, ticketWorkFirstSeen(tk)
        End If
    Next k
    ...
    For di = 1 To nd
        ...
        For Each k In dayTickets.Keys
            If ticketCreated.Exists(CStr(k)) Then
    ...
    Call StyleSummaryLabel(wsWR.Cells(r, 1))
    wsWR.Range("E" & r & ":F" & r).Merge
    wsWR.Cells(r, 5).Formula = "=SUMPRODUCT((Data!$J$2:$J$" & lastDataRow & "=1)*" & _
        "(Data!$K$2:$K$" & lastDataRow & "<>"""")*" & _
        "(Data!$K$2:$K$" & lastDataRow & ">=DATE(" & Year(weekStart) & "," & Month(weekStart) & "," & Day(weekStart) & "))*" & _
        "(Data!$K$2:$K$" & lastDataRow & "<=DATE(" & Year(weekEnd) & "," & Month(weekEnd) & "," & Day(weekEnd) & ")))"
    Call StyleSummaryValue(wsWR.Cells(r, 5), navy)
    r = r + 1
    ...
    Call StyleSummaryLabel(wsWR.Cells(r, 1))
    wsWR.Range("E" & r & ":F" & r).Merge
    wsWR.Cells(r, 5).Formula = "=SUMPRODUCT(1/COUNTIF(Data!E2:E" & lastDataRow & ",Data!E2:E" & lastDataRow & "))"
    Call StyleSummaryValue(wsWR.Cells(r, 5), navy)
    r = r + 2
